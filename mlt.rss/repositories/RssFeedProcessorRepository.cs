...
internal class RssFeedProcessorRepository : IRssFeedProcessorRepository
{
    ...
    public async Task<RssSyncResult> ProcessFeed(string rssFeedId)
    {
        ...
        var doc = await HttpRequestHelper.DoActionWithRetry(async () =>
                                                            {
                                                                using var reader = XmlReader.Create(rssFeed.Url, new XmlReaderSettings { Async = true });
    ...
    private static Func<XElement, bool> FilterExistingElements(List<RssFeedResult> existingFeedResults, XNamespace tv, XNamespace nyaa)
        => xElement => !existingFeedResults.Any(feedResult => (!string.IsNullOrEmpty(feedResult.TvInfoHash) && feedResult.TvInfoHash == xElement.Element(tv + "info_hash")?.Value)
                                                           || (!string.IsNullOrEmpty(feedResult.NyaaInfoHash)
                                                            && feedResult.NyaaInfoHash == xElement.Element(nyaa + "infoHash")?.Value));
}